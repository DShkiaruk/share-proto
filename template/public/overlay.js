...
(() => {
  ...
  const ICONS = {
    ...
    image: svg(
      '<rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="9" cy="9" r="1.6"/><path d="m21 15-4.5-4.5L7 20"/>'
    ),
  ...
  function openThread(id, pinEl) {
    ...
    // A hidden pin measures 0×0 at the origin; using that dropped the card in
    // the top-left corner, over the sidebar, nowhere near the comment.
    const pos = pinEl?.getClientRects().length ? pinEl.getBoundingClientRect() : null;
  ...
  function renderMap(fit = false) {
    ...
    for (const n of model.nodes) {
      ...
      /* Actions on the card. A comment made here belongs to the screen rather
         than to a spot on it — "this step shouldn't be here" is about the
         screen, and you cannot point at a pixel of a screen you are not on. */
      const acts = el('div', 'map-actions');
      const cbtn = el('button', 'map-act');
      cbtn.append(icon('comment'), el('span', null, 'Comment'));
      ...
      if (!n.shot && state.role === 'designer') {
        // "No picture yet" is a gap the designer can close on the spot, not a
        // status they have to live with.
        const add = el('button', 'map-act');
        add.append(icon('image'), el('span', null, 'Add picture'));
        add.title = `Add a picture for ${n.name}`;
        const file = el('input');
        file.type = 'file';
        file.accept = 'image/*';
        ...
        file.addEventListener('change', async () => {
          ...
          try {
            ...
            await api('POST', { action: 'shot', label: n.label, image });
            await refresh();
            renderMap();
          } catch {
            add.disabled = false;
            toast('Couldn’t add that picture');
          }
        });
        add.addEventListener('click', (e) => {
          e.stopPropagation();
        ...
        acts.append(add, file);
      }
      const foot = el('div', 'map-foot');
      foot.append(name, meta, acts);
